...
**Assign Teacher tab:**
- Left panel: filter to unassigned students from `GET /admin/students?type=personal_tutor` (filter where `assigned === false`)
...
- "Assign" button (disabled until both selected) → `POST /admin/assign/teacher`
  - body: `{ personal_tutor_id, teacher_professional_id }`
- On success: both lists refresh, toast "Teacher assigned successfully"

**Assign Trainer tab:** Same layout.
- Left panel: unassigned students from `GET /admin/students?type=individual_coaching` (filter where `assigned === false`)
- Right panel: `GET /admin/professionals/available?type=trainer`
- On assign: `POST /admin/assign/trainer`
  - body: `{ individual_participant_id, trainer_professional_id }`
...
└── Withdrawals              ← NEW
```
## New Endpoints (Sessions, Assignments Overview, Batches per Society/School)
...
```
GET  /api/v1/admin/fee-structures                           # all sections
GET  /api/v1/admin/fee-structures?section=school|society|individual_coaching|personal_tutor
PUT  /api/v1/admin/fee-structures/:id                       # edit existing fee row
POST /api/v1/admin/fee-structures                           # create new fee row
```
...
```json
{
  ...
  "term_months": 3,
  "total_fee": 2500,
  "effective_monthly": 833
}
```

**Fee Structures page UX:** Show as a table grouped by section tab (Society / School / Individual Coaching / Personal Tutor). Each row has an inline "Edit" button that opens a small modal with `total_fee` and `effective_monthly` fields pre-filled. On save → `PUT /fee-structures/:id`.

## Custom Fee Structures

Admin can create custom-named fee tiers for society and school (e.g. when a society negotiates a special rate). Uses `society_category: "custom"` + a `custom_category_name` string label.
...
{
  "activity_id": 2,
  "coaching_type": "group_coaching",      // society
  "society_category": "custom",
  "custom_category_name": "Green Acres Special",  // required when custom
  "term_months": 3,
  "total_fee": 4500,
  "effective_monthly": 1500
}
```
- `coaching_type: "school_student"` for school custom tiers
- `custom_category_name` is **required** when `society_category = "custom"` → returns 400 otherwise

### Fee structure response shape (section=society)
```json
{
  "activity_id": 2,
...
### Fee structure response shape (section=school)
```json
{
  "activity_id": 1,
